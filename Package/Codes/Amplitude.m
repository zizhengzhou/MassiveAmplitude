...
ReduceRules[Np_] :=
    Join[ruleP1[Np], ruleP2[Np], ruleP3[Np], ruleSchA, ruleSchS, If[$BHOmitLowDim,RuleOmitLowDim[Np],{}]];
    ...
    Module[
      {
        ...
        matchedCFCoordinateDict, totalCF, totalCoordinate, independentIndex, metaInfo, sortFun, getCoordinate},
      np = Length@spins;
      masses = MassOption[OptionValue@mass, np];
      ...
      (* For stable sort*)
      sortFun[amp_] := If[MemberQ[bhs, amp],
        (* For BH code dim = fake dim, physical dim = fake dim*)
        100 * CalcAmpDim[fakeDim, spins] + fakeDim,
        100 * CalcAmpDim[cfsDictReverse[amp][[1]], spins] +
            CalcPhysicalDim[spins, cfsDictReverse[amp][[1]], cfsDictReverse[amp][[2]], masses]
      ];
      getCoordinate[amp_] := If[MemberQ[bhs, amp],
        Module[{i = FirstPosition[bhs, amp], v = Table[0, Length@bhs]}, v[[i]] = 1; v],
        matchedCFCoordinateDict[MatchCFDim[np, matchOpts]@amp]
      ];

      totalCF = Keys@cfsDictReverse ~ Join ~ bhs;
      totalCF = SortBy[totalCF, sortFun];
      totalCoordinate = getCoordinate /@ totalCF;
