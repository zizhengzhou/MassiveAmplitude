...
BasisByModel[particlesParm_List, fromOpDim_, toOpDim_, OptionsPattern[]] := Module[
  ...
  If[MatchQ[OptionValue@output, "amplitude" | "amp"],
    resultDict["particleId"] = MapThread[#1 -> #2&, {particles, Range[Length@particles]}];
    If[Length@DeleteCases[""][colors]>0,
      resultDict["su3Id"] = #[[1]] -> GetColorIndDict[su3ShapeDict /@ colors][#[[2]]] & /@
          Transpose[{particles, Range[Length@particles]}] //
...
OrganizeResultTexDict[texDict_?AssociationQ, OptionsPattern[]] := Module[
  {
    EquLine, DimHeadLine,
    keys = texDict // Keys // Cases[_?IntegerQ] // Sort ,
...
BasisResultDict2JsonObj[particles_List, texDict_?AssociationQ, OptionsPattern[]] := Module[
  ...
  dims = Keys@texDict // Cases[_?IntegerQ];
